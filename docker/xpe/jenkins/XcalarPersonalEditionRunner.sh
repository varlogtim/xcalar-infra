...
XPE_GUI_BUILD_DIR="${XPE_GUI_BUILD_DIR:-"$XLRGUIDIR/$XPE_BUILD_TARGET_DIR"}" # custom gui build for xdee; will build in XLRGUIDIR if this dir doesn't exist

APP_NAME="${APP_NAME:-"Xcalar Desktop Edition"}" # basename for the app
APPTARFILE="${APPTARFILE:-"$APP_NAME.tar.gz"}" # name of final tarred app that ends up in bld
DMGNAME="${DMGNAME:-"$APP_NAME.dmg"}" # will create script to be run post-bld to generate dmg for the app; script will specify to name the dmg this

# some app metadata and build files require the app's name;
# have put a common placeholder value in those files, and will sed
...
buildXcalarGuiForApp() {
    ...
    if [ ! -d "$XPE_GUI_BUILD_DIR" ]; then
        ...
        node_modules/grunt/bin/grunt dev --product=XPE --branding="$APP_NAME"
        ...
        if [ ! -d "$XPE_BUILD_TARGET_DIR" ]; then
            ...
            XPE_GUI_BUILD_DIR=$(pwd)
        fi
    fi

    # build the xpe server
    cd "$XPE_GUI_BUILD_DIR/services/xpeServer"
    npm install

    # modify GUI code to ignore version mismatch, if requested
    if "$IGNORE_VERSION_MISMATCH"; then
        sed -i 's/versionMatch\s*=\s*false/versionMatch = true/g' ${XPE_GUI_BUILD_DIR}/assets/js/shared/setup/xvm.js
    fi
}
...
generate_app_assets()  {
    ...
    make docker-image INSTALLER_PATH="$PATH_TO_XCALAR_INSTALLER" CONTAINER_NAME="$XCALAR_CONTAINER_NAME" CONTAINER_IMAGE="$XCALAR_IMAGE_NAME" PORT_MAPPING="" CUSTOM_GUI="$XPE_GUI_BUILD_DIR"
...
build_app() {
    ...
    tar -czf "$APPTARFILE" "$APP_NAME.app"
...
build_app
setup_files_for_drag_and_drop_dmg
set_app_name "$FINALDEST" # set_app_name already ran on .app; app is tarred now. this is handling any non-app files in the build (like drag and drop dmg files)
