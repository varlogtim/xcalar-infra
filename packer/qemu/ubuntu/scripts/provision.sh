#!/bin/bash
# DO NOT EDIT! THIS FILE IS AUTOGENERATED BY bin/docker2shell.sh!
...
cd $DOCKERPWD && apt-get update -y && DEBIAN_FRONTEND=noninteractive http_proxy=$APT_PROXY apt-get install -y thrift-dev libhdfs3-dev pmd || exit $?
...
cd $SRCDIR && curl -sL http://repo.xcalar.net/data/yelp.tar.gz | tar zxf - -C /var/tmp && cd - || exit $?
cd $SRCDIR && curl -sL http://repo.xcalar.net/data/gdelt-small.tar.gz | tar zxf - -C /var/tmp && cd - || exit $?

cd $SRCDIR && curl -sL http://repo.xcalar.net/scripts/install_unixOdbc.sh > /usr/local/bin/install_unixOdbc.sh &&  cd - || exit $?
cd $SRCDIR && curl -sL http://repo.xcalar.net/scripts/setupOdbcMysql.sh > /usr/local/bin/setupOdbcMysql.sh && cd - || exit $?
...
cd $SRCDIR && curl -sL http://repo.xcalar.net/patches/conf/xcalar-sysctl.conf > /etc/sysctl.d/99-xcalar.conf && cd - || exit $?
cd $SRCDIR && curl -sL http://repo.xcalar.net/patches/conf/xcalar-limits.conf > /etc/security/limits.d/99-xcalar.conf && cd - || exit $?
cd $SRCDIR && curl -sL http://repo.xcalar.net/patches/conf/xcalar-logrotate.conf > /etc/logrotate.d/xcalar && cd - || exit $?
cd $SRCDIR && curl -sL http://repo.xcalar.net/patches/conf/xcalar-rsyslog.conf > /etc/rsyslog.d/42-xcalar.conf && cd - || exit $?
...
# EXPOSE 80 443 9090 5000 8000 8080
