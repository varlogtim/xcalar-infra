...
readonly WD=${PWD}
ARGS=("$@")
LAYER_NAME=${1?Must specify layer name}
...
if [ -z "$container" ]; then
    docker run --rm -e container=docker -v $WD:/var/task:z -w /var/task lambci/lambda:build-${RUNTIME} bash -x $(basename "${BASH_SOURCE[0]}") "${ARGS[@]}"
...
rm -f ${ZIP_ARTIFACT}
cd ${LAYER_BUILD_DIR}/..
zip -9r -q ${WD}/${ZIP_ARTIFACT} ./python/
cd - >/dev/null
chown $(stat -c %u $WD) ${WD}/${ZIP_ARTIFACT} .
