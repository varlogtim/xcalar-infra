...
usage() {
    echo >&2 "usage: $0 [-i <inpute-node-installer.sh>] [-o <installer-output.sh>] [--osext <platform to extract (default: $OSEXT)>]"
    exit 1
}
...
main () {
    ...
    extract "$INSTALLER" | tar zxf - -C $TMPDIR --exclude "*${EXCOS[0]}*" --exclude "*${EXCOS[1]}*" --exclude "*${EXCOS[2]}*" >&2
    echo "Creating new archive $OUTPUT ..." >&2
    (cat $TMPDIR/xcalar-install-top.sh; tar czf - -C $TMPDIR . --transform=s,^./,,g) > "$OUTPUT"
    chmod 0555 "$OUTPUT"
    rm -f $TMPDIR/xcalar-install-top.sh*
    ...
    if [ -n "$UPLOAD" ]; then
        for REPO in ${UPLOAD//,/ }; do
            installer-url.sh -d $REPO $INSTALLER
        done
    fi
    return $rc
}
