...
usage () {
  ...
  PARAMETER_KEYS=$(< $TEMPLATE jq -r '.parameters|keys|@tsv')
  for param in ${PARAMETER_KEYS}; do
    DESC="$(< $TEMPLATE jq -r '.parameters.'$param'.metadata.description')"
    ...
    case "$cmd" in
        *.zip)
        ...
        deploy)
            ...
            (
            debug az group deployment validate --template-uri "${templateUrl}" --parameters @$PARAMETERS --parameters _artifactsLocation="$artifactsLoc" --parameters _artifactsLocationSasToken='' \
            ...
            debug az group deployment create  -g "${GROUP}" --name "${DEPLOY}" --no-wait -ojson --template-uri "${templateUrl}" --parameters @$PARAMETERS --parameters _artifactsLocation="$artifactsLoc" _artifactsLocationSasToken='' \
                            "location=${LOCATION}" appName=${CLUSTER} "${EXTRA_ARGS[@]}" > $TMPDIR/error.json
            debug az group deployment wait --exists -g "$GROUP" --name "$DEPLOY"
