...
while [ $# -gt 0 ]; do
    ...
    shift
    case "$cmd" in
        --nfs-mount-target) EFSMOUNT="$1"; shift;;
        ...
        --installerurl) test -z "$1" || INSTALLERURL="$1"; shift;;
...
case "$AWS_DEFAULT_REGION" in
    us-west-2)
        test -n "$EFSMOUNT" || EFSMOUNT="netstore.${AWS_DEFAULT_REGION}.aws.xcalar.com"
        ;;
    us-east-1)
        test -n "$EFSMOUNT" || EFSMOUNT="netstore.${AWS_DEFAULT_REGION}.aws.xcalar.com"
        ;;
...
if ! rpm -q xcalar; then
    ...
    if ! test -s installer.sh; then
        if [[ "$INSTALLERURL" =~ ^http ]]; then
            curl -fL "$INSTALLERURL" -o installer.sh
        elif [[ "$INSTALLERURL" =~ s3:// ]]; then
            aws s3 cp "$INSTALLERURL" installer.sh
        fi
    fi
    if test -s installer.sh; then
        bash -x installer.sh --nostart --startonboot 2>&1 | tee installer.log
        if [ ${PIPESTATUS[0]} -ne 0 ]; then
            echo "Failed to install ${PWD}/installer.sh (from $INSTALLERURL)"
            exit 1
        fi
        touch /var/lib/xcalar-installer/finished
    fi
...
mkdir -p /netstore
if [ -n "$EFSMOUNT" ]; then
    echo "${EFSMOUNT}:/ /netstore nfs4 nfsvers=4.1,rsize=1048576,wsize=1048576,hard,timeo=600,retrans=2	0	0" >>/etc/fstab
    mount /netstore
fi
...
if [ $NUM_INSTANCES -gt 1 ]; then
    ...
    echo "${EFSMOUNT}:/cluster/$CLUSTER_ID ${XLRROOT} nfs4 nfsvers=4.1,rsize=1048576,wsize=1048576,hard,timeo=600,retrans=2	0	0" >>/etc/fstab
    mkdir -p ${XLRROOT}
    mount ${XLRROOT}
else
    XLRROOT=/var/opt/xcalar
