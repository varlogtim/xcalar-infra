...
case "${CLOUD_PROVIDER}" in
    aws)
        ...
        DOCKER_INSTALL_HOST_IMAGE=${DOCKER_INSTALL_HOST_IMAGE:-ami-4185e621}
        DOCKER_INSTALL_MACHINE_TYPE=${DOCKER_INSTALL_MACHINE_TYPE:-r4.2xlarge}
        TESTHOST_IMAGE=${TESTHOST_IMAGE:-ami-4185e621}
        TESTHOST_MACHINE_TYPE=${TESTHOST_MACHINE_TYPE:-r4.2xlarge}
        TESTCLUSTER_IMAGE="${BUILD_CLUSTER_IMAGE}"
        TESTCLUSTER_MACHINE_TYPE=${TESTCLUSTER_MACHINE_TYPE:-r4.2xlarge}
        ;;
    gce)
        BUILD_USERNAME=${BUILD_USERNAME:-"$(id -un)"}
        DOCKER_INSTALL_HOST_IMAGE=${DOCKER_INSTALL_HOST_IMAGE:-centos-7-v20170227}
        DOCKER_INSTALL_MACHINE_TYPE=${DOCKER_INSTALL_MACHINE_TYPE:-g1-small}
...
cat <<EOF
{
    ...
    "DockerInstallHostConfig": {
        "OSVersion": "CENTOS7",
        "OSImage": "${DOCKER_INSTALL_HOST_IMAGE}",
        "OSProject": "centos-cloud",
        "DockerSource": "RPM",
        "MachineType": "${DOCKER_INSTALL_MACHINE_TYPE}"
    },
    "TestHostConfig": {
        "OSVersion": "CENTOS7",
