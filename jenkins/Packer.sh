...
do_packer() {
    ...
    bash -x ../build.sh --template $PACKERCONFIG --installer-url "$INSTALLER_URL" -- ${BUILDER:+-only=${BUILDER}} ${PRODUCT:+-var product=$PRODUCT }-var project=${PROJECT} -var license="${LICENSE}" -var disk_size=$DISK_SIZE 2>&1 | tee $OUTDIR/output.txt
