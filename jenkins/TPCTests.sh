...
if [ -z $JDBC_HOST ]; then
    say "ERROR: JDBC_HOST cannot be empty"
    exit 1
fi
...
# Common arguments...
ARGS=" --test_id=$TEST_ID --test_jdbc_path=$TEST_JDBC_PATH --jdbc_host=$JDBC_HOST --jdbc_port=$JDBC_PORT"
ARGS="$ARGS --api_port=$API_PORT --user=$USER --pass=$PASS"
