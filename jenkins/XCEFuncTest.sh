...
do
    LD_PRELOAD=$XLRDIR/src/lib/libfaulthandler/.libs/libfaulthandler.so.0 $XLRDIR/src/bin/usrnode/usrnode -f $configFile-copy -i $ii -n $numNode > node.${ii}.log 2>&1  &
    $XLRDIR/src/bin/monitor/xcmonitor -n $ii -c $configFile-copy >xcmonitor.${ii}.log 2>&1  &
done
...
echo "Serialization stats"
$XLRDIR/src/bin/cli/xccli -c "stats 0 XdbMgr"
