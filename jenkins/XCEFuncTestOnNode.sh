...
sed -i -e "s'Constants\.BufferCachePercentOfTotalMem=.*'Constants\.BufferCachePercentOfTotalMem=$BufferCachePercentOfTotalMem'" $configFile-copy

for ii in `seq 0 $((numNode - 1))`;
do
    LD_PRELOAD=$XLRDIR/src/lib/libfaulthandler/.libs/libfaulthandler.so.0 usrnode -f $configFile-copy -i $ii -n $numNode  &> /dev/null &
    xcmonitor -n $ii -c $configFile-copy &> /dev/null &
done
...
export XCE_CONFIG=${configFile}-copy
python2.7 $ScriptPath $TestStr --cliPath xccli --cfgPath $configFile-copy --single
